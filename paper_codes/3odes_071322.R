...
bivalent_full_ka0_objective <- function(pars, df, incl_concentrations, num_conc){
  ...
  for (i in 1:num_conc){
    ...
    #y_asc <- approx(t_asc_sim, y_asc_sim, t_asc)$y
    # Dissociation
...
fit_bivalent_full <- function(well_idx, sample_info, x_vals, y_vals,
  ...
  # lower <- c(0, 0, 1e-5, 1e-5, rep(0, num_conc), rep(-Inf, num_conc))
  ...
  Rmax <- pars[5:14]
  #RI <- pars[10:14]
  fit_outcomes <- get_fit_outcomes(Rmax, ka, kd, df, num_conc,
                                   incl_concentrations,
                                   t0 = t0_start)
...
get_fit_outcomes <- function(Rmax, ka, kd, df, num_conc,
                             incl_concentrations,
                             t0){
  ka1 <- ka[1]
  ka2 <- ka[2]
  ...
  for (i in 1:num_conc){
    ...
    state <- c(L  = Rmax[i],
               X1 = 0,
               X2 = 0)
    RI <- Rmax[num_conc+i] #Rmax[num_conc+i] mean(df_assoc$RU[1:3])
    t_asc <- df_assoc$Time # - df_assoc$Time[1]
    out <- ode(y = state, times = t_asc, func = bivalent_model, parms = parameters)
    df_assoc$RU <- out[,3] + out[,4] + RI
    
    # Dissociation
    t_dis <- df_dissoc$Time #- df_dissoc$Time[1]
    ##############################################################################
    ##############################################################################
    # df_dissoc$RU <- out[length(out[,3]),3]*exp(-kd1*t_dis) + out[length(out[,4]),4]*exp(-(kd2)*t_dis) + RI
    ##############################################################################
    parameters <- c(ka1 = 0,
                    kd1 = kd1,
                    ka2 = 0,
  ...
  # bind_rows(df_assoc, df_dissoc) %>% geom_line(mapping = aes(x = Time, y = RU))
...
plot_sensorgrams_with_fits <- function(well_idx, sample_info, fits, x_vals, y_vals, 
  ...
  sub_title <- paste("Bivalent Analyte Model-2 Fitting with Nominal Length of Dissociation") #paste("1:1 Langmuir Model Fitting with Extended Length of Dissociation")
...
plot_sensorgrams_with_monovalent_fits <- function(well_idx, sample_info, fits, x_vals, y_vals, 
                                                  incl_conc_values, n_time_points){
  
  if (!is.null(fits[[well_idx]]$result$error))
    return(NULL)
  print(well_idx)
  start_idx <- sample_info[well_idx,]$FirstInclConcIdx
  num_conc <- sample_info[well_idx,]$NumInclConc
  ...
  fit_df <- fits[[well_idx]]$result$FitOutcomes
  #fit_df <- fits[[well_idx]]$FitOutcomes
  ...
  sub_title <- paste("1:1 Langmuir Model with Bulk Shift Fitting with Extended Length of Dissociation")
  
  dissociation_start <- sample_info$Baseline[[well_idx]] + sample_info$`Bsl Start`[[well_idx]] + sample_info$Association[[well_idx]]
  dissociation_start <- dissociation_start - baseline
  plot <- ggplot(df, aes(x = Time, y = RU)) + geom_point(size = 0.09, aes(color = Concentration)) #+
  #geom_line(data =  fit_df, aes(x = Time, y = RU, group = Concentration), color = "black", size=1) +
  geom_vline(xintercept = dissociation_start, linetype="dashed",
             color = "black", size=1)
