...
# sample_sheet_path <- "paper_data2/Bivalent Binding 2nd - CH505 CH31 - info sheet short.csv"
...
# data_file_path <- "paper_data2/Bivalent Binding 3rd - CH505 CH31 - downselect newformat.xlsx"
...
# save(fits_list, file="bivalent_oldResults_t0_short2_nonRegen2.Rdata")
load(file='bivalent_oldResults_t0_short2_nonRegen2.Rdata')
...
for (well_idx in 1:nwells){
  ...
  if (!is.null(plot_bivalent_fit[[well_idx]])){
    ...
    data_file_name <- paste("paper_codes/figures/bivalent_old_short2/fitbivalent_old_nonRegen_short2",well_idx,".png",sep="")
    ggsave(filename = data_file_name, plot=plot_bivalent_fit[[well_idx]], width = 12, height = 7.31)
  }
}
dev.off()
...
pdf(file = "paper_codes/figures/bivalent_old_short2/Antigen2_out_bivalent_new2.pdf")
for (well_idx in 1:nwells){
  print(well_idx)
