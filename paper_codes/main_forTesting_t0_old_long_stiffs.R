...
save(fits_list, file="bivalent_oldResults_t0_long_stiff.Rdata")
# load(file='NoBulkshiftShort.Rdata')
load(file='bivalent_oldResults_t0_long_stiff.Rdata')
...
for (well_idx in 1:nwells){
  ...
  if (!is.null(plot_bivalent_fit[[well_idx]])){
    grid.arrange(plot_bivalent_fit[[well_idx]])
    data_file_name <- paste("paper_codes/figures/bivalent_old_long_stiff/fitbivalent_old_long",well_idx,".png",sep="")
    ggsave(filename = data_file_name, plot=plot_bivalent_fit[[well_idx]])
  }
}
dev.off()
...
pdf(file = "paper_codes/figures/bivalent_old_long_stiff/Antigen2_out_bivalent_new.pdf")
for (well_idx in 1:nwells){
  print(well_idx)
