...
fit_association_dissociation <- function(well_idx, sample_info, x_vals, y_vals,
  ...
  for (i in 1:num_run){
    ...
    init_params <- c(init_ks, init_Rmaxs, runif(num_conc, min=0, max=120))
    
    init_params_list <- rbind(init_params_list, init_params)
  ...
  write_csv(full_param_table, paste('~/Summer2022/project/hiv-summer-2022/paper_codes/param_table_new_100Ran/full_param',as.character(well_idx),'.csv',sep=''))
