...
save(fits_list, file="bivalent_oldResults_AL20_lRmax_short.Rdata")
...
for (well_idx in 1:nwells){
  ...
  if (!is.null(plot_bivalent_fit[[well_idx]])){
    grid.arrange(plot_bivalent_fit[[well_idx]])
    data_file_name <- paste("figures/bivalent_old_short/fitbivalent_old_short",well_idx,".png",sep="")
    ggsave(filename = data_file_name, plot=plot_bivalent_fit[[well_idx]])
  }
}
dev.off()
...
pdf(file = "figures/bivalent_old_short/Antigen2_out_bivalent_new.pdf")
for (well_idx in 1:nwells){
  print(well_idx)
