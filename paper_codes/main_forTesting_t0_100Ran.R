...
save(fits_list, file="bivalent_newResults_t0_300Ran_long2.Rdata")
# load(file='NoBulkshiftShort.Rdata')
load(file='bivalent_newResults_t0_100Ran_long.Rdata')
...
for (well_idx in 1:nwells){
  ...
  if (!is.null(plot_bivalent_fit[[well_idx]])){
    grid.arrange(plot_bivalent_fit[[well_idx]])
    data_file_name <- paste("paper_codes/figures/bivalent_new_long_lRmax/fitbivalent_new_long",well_idx,".png",sep="")
    ggsave(filename = data_file_name, plot=plot_bivalent_fit[[well_idx]])
  }
}
dev.off()
