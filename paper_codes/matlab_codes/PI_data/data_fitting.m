...
RU0s = RU(1,:);
% Chosen parameters
IC_params = [1e3, 1e-4, 1e-4, 1e-4 RUmaxs, RU0s];%[1e1, 1e-2, 1e-2, 1e-2, RUmaxs, RU0s];
LB = [1e0, 1e-8, 1e-8, 0, zeros(size(RUmaxs),'like', RUmaxs), zeros(size(RU0s),'like', RU0s)]; %zeros(size(IC_params),'like', IC_params);
UB = [1e8, 1e-1, 1e-1, 1e-1, 2*RUmaxs, 2*RU0s];%[1e6, 1e-1, 1e-2, 1e-2];

[params,fval] = fmincon(@(params) objective(Time,RU,params,concs,baseline,association,dissociation), IC_params,[],[],[],[],LB,[],[]);
...
for i=1:length(concs)  
    ...
    y_pred = R0 + [y_asc(:,2) + y_asc(:,3); y_dis(:,2) + y_dis(:,3)];
