...
options = optimset('Algorithm','levenberg-marquardt','MaxIter',1e4,'TolFun',1e-8,'TolX',1e-8);
...
for choose_kd2 = choose_kd2s
    for choose_IC = choose_ICs
        if choose_kd2 == 12
            load('generated_data_N5_4000T/data_12.mat');
            kd2_dir = 'generated_data_N5_4000T/result_12/';
        elseif choose_kd2 == 5
            load('generated_data_N5_4000T/data_5.mat');
            kd2_dir = 'generated_data_N5_4000T/result_5/';
        elseif choose_kd2 == 4
            load('generated_data_N5_4000T/data_4.mat');
            kd2_dir = 'generated_data_N5_4000T/result_4/';
        elseif choose_kd2 == 3
            load('generated_data_N5_4000T/data_3.mat');
            kd2_dir = 'generated_data_N5_4000T/result_3/';
        elseif choose_kd2 == 2
            load('generated_data_N5_4000T/data_2.mat');
            kd2_dir = 'generated_data_N5_4000T/result_2/';
        else
            load('generated_data_N5_4000T/data_1.mat');
            kd2_dir = 'generated_data_N5_4000T/result_1/';
        end
        ...
        if choose_IC == 1
            IC_params = [1e3, 1e-1, 1e-3, 1e-3, RUmaxs, RU0s];%[1e1, 1e-2, 1e-2, 1e-2, RUmaxs, RU0s];
        elseif choose_IC == 2
            IC_params = [1e3, 1e-2, 1e-3, 1e-4, RUmaxs, RU0s];
        elseif choose_IC == 3
            IC_params = [1e3, 1e-3, 1e-3, 1e-5, RUmaxs, RU0s];
        elseif choose_IC == 4
            IC_params = [1e3, 1e-4, 1e-3, 1e-6, RUmaxs, RU0s];
        elseif choose_IC == 5
            IC_params = [1e3, 1e-5, 1e-3, 1e-7, RUmaxs, RU0s];
        else
            IC_params = [1e3, 1e-6, 1e-3, 1e-8, RUmaxs, RU0s];
        end
        ...
        params = lsqnonlin(@(params) objective(t,Ydata,params,concs,R0s,Rmaxs), IC_params,LB,[], options);
        ...
        hold off
%         saveas(gcf,save_fig)
%         save(save_result,'params','IC_params','true_params','Rmaxs','R0s')
%         close all;
    end
end
...
t_asc = 120:2:420;
t_dis = 420:10:4000; %423:10:4000; %423:5:1400; %423:30:7200;
t = [t_asc, t_dis];
