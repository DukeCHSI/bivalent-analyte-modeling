...
sample_sheet_path <- "~/Summer2022/project/paper_data/Bivalent Binding 2nd - CH505 CH31 - info sheet.csv"
...
save(fits_list, file="bivalent_newResults_AL20_lRmax_long.Rdata")
...
for (well_idx in 1:nwells){
  ...
  if (!is.null(plot_bivalent_fit[[well_idx]])){
    grid.arrange(plot_bivalent_fit[[well_idx]])
    data_file_name <- paste("figures/bivalent_new_long/fitbivalent_new_long",well_idx,".png",sep="")
    ggsave(filename = data_file_name, plot=plot_bivalent_fit[[well_idx]])
  }
}
dev.off()
...
pdf(file = "figures/bivalent_new_long/Antigen2_out_bivalent_new.pdf")
for (well_idx in 1:nwells){
  print(well_idx)
