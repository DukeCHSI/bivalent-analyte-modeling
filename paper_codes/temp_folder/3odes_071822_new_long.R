...
fit_association_dissociation <- function(well_idx, sample_info, x_vals, y_vals,
  ...
  use_secondRebind <- "Y" #sample_info[well_idx,]$`Sec. Rebinding`
  ...
  if (model == 'bivalentAnalyte_PredAsc'){
    ...
    upper <- NULL
    param_names <- c("ka1", "ka2", "kd1", "kd2", rep("Rmax", num_Rmax), rep("R0", num_R0), rep('t_star', num_conc))
  }
    ...
    if (use_globalRmax == "N"){
      ...
      lower <- c(0, 0, 0, 0, rep(max(R0_start$min), num_Rmax), rep(0, num_R0), rep(0, num_conc))
      upper <- c(1e8, 1e-1, 1e-1, 1e-1, rep(5*max(Rmax_start$max), num_Rmax), rep(max(R0_start$min), num_R0), rep(1, num_conc))
    }
    ...
    for (ka2_IG in ka2_IGs){
      for (kd1_IG in kd1_IGs){
        for (kd2_IG in kd2_IGs){
          # temp_IGs <- c(ka1_IG, ka2_IG, kd1_IG, kd2_IG, rep(max(Rmax_start$max), num_Rmax), rep(0.5, num_conc))
          temp_IGs <- c(ka1_IG, ka2_IG, kd1_IG, kd2_IG, rep(max(Rmax_start$max), num_Rmax), rep(60, num_conc))
  ...
  for (i in 1:num_run){
    ...
    tryCatch(
      expr = {
        ...
        print(coefficients(best_res_R0))
      },
      error = function(cond){
  ...
  write_csv(full_param_table, paste('~/Summer2022/project/hiv-summer-2022/paper_codes/param_table_new_long_gRmax/full_param',as.character(well_idx),'.csv',sep=''))
...
plot_sensorgrams_with_fits <- function(well_idx, sample_info, fits, x_vals, y_vals, 
  ...
  sub_title <- paste("Bivalent Analyte Model-2 with Nominal Length of Dissociation")
