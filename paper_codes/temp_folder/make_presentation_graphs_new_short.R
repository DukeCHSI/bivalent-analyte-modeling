...
library(parallel)
library(svDialogs)
setwd("~/Summer2022/project/hiv-summer-2022/paper_codes")
...
#sample_sheet_path <- "~/Google Drive/My Drive/R/HIV_new_data/CH31 Binding Data/CH505 - info sheet.csv"
#sample_sheet_path <- "~/Google Drive/My Drive/R/HIV_new_data/Bivalent Data/biphasic example - info sheet.csv"
#sample_sheet_path <- "~/Google Drive/My Drive/R/HIV_new_data/digitized data/digitized data example 1 info sheet.csv"
#sample_sheet_path <- "~/Google Drive/My Drive/R/HIV_new_data/CH505 Bivalent/Bivalent Binding - CH505 CH31 - info sheet changed.csv"
sample_sheet_path <- "~/Summer2022/project/hiv-summer-2022/paper_data/Bivalent Binding 2nd - CH505 CH31 - info sheet short.csv"
...
#data_file_path <- "~/Google Drive/My Drive/R/HIV_new_data/CH31 Binding Data/CH505 - new output format.xlsx"
#data_file_path <- "~/Google Drive/My Drive/R/HIV_new_data/Bivalent Data/biphasic example newformat_CORRECT.xlsx"
#data_file_path <- "~/Google Drive/My Drive/R/HIV_new_data/digitized data/digitized data example 1 newformat v3 copy.xlsx"
#data_file_path <- "~/Google Drive/My Drive/R/HIV_new_data/CH505 Bivalent/Bivalent Binding - CH505 CH31 - new output format_aligned.xlsx"
data_file_path <- "~/Summer2022/project/hiv-summer-2022/paper_data/Bivalent Binding 2nd - CH505 CH31 - new output format.xlsx"
...
#plot_list_out <- map(.x = 1:nwells, .f = plot_sensorgrams_with_fits, 
#                     sample_info, fits_list_kd, 
#                     Time[, keep_concentrations], RU[, keep_concentrations],
#                     incl_concentrations_values, 
#                     n_time_points)
...
# bivalent_fits_list <- mclapply(X = 1:nwells, FUN = fit_bivalent_full, mc.cores = num_cores, sample_info,
#                       Time[, keep_concentrations],
#                       RU[, keep_concentrations],
#                       incl_concentrations_values)
...
save(fits_list, file="bivalent_newResults_AL20_gRmax_short.Rdata")
# load(file='NoBulkshiftShort.Rdata')
# load(file='bivalent_newResults_AL20_lRmax_short.Rdata')
...
for (well_idx in 1:nwells){
  ...
  if (!is.null(plot_bivalent_fit[[well_idx]])){
    grid.arrange(plot_bivalent_fit[[well_idx]])
    data_file_name <- paste("figures/bivalent_new_short_gRmax/fitbivalent_new_short",well_idx,".png",sep="")
    ggsave(filename = data_file_name, plot=plot_bivalent_fit[[well_idx]])
  }
}
dev.off()
...
pdf(file = "figures/bivalent_new_short_gRmax/Antigen2_out_bivalent_new.pdf")
for (well_idx in 1:nwells){
  print(well_idx)
